...
class PromisZone(BaseVolumeDataset):
    ...
    def __getitem__(self, idx):
        ...
        zone_mask = self.read(path["zone_mask"]).unsqueeze(0)

        label_str = path["zone_label"]
        zone_level = [int(float(x)) for x in ast.literal_eval(label_str)]
        zone_level = torch.tensor(zone_level)
...
def build_UCL_loader(args):
    ...
    if args.data_num > 0:
        train_set = UCLSet(args, f"spilt/UCL/train_{args.data_num}.csv", 'train', train_transforms)
    else:
        train_set = UCLSet(args, "spilt/UCL/train.csv", 'train', train_transforms)
...
def build_Anatomy_loader(args):
    ...
    if args.data_num > 0:
        train_set = AnatomySet(args, f"spilt/anatomy/train_{args.data_num}.csv", 'train', train_transforms)
    else:
        train_set = AnatomySet(args, "spilt/anatomy/train.csv", 'train', train_transforms)
...
def build_PromisZone_loader(args):
    train_transforms, val_transforms, test_transforms = get_transforms(args)
    train_set = PromisZone(args, "spilt/promis_zone_recurated/train.csv", 'train', train_transforms)
    val_set = PromisZone(args, "spilt/promis_zone_recurated/val.csv", 'val', val_transforms)
    test_set = PromisZone(args, "spilt/promis_zone_recurated/test.csv", 'test', test_transforms)
    train_loader = DataLoader(
        train_set,
        batch_size=args.batch_size,
        shuffle=True,
...
def build_PromisZonedef2_loader(args):
    train_transforms, val_transforms, test_transforms = get_transforms(args)
    train_set = PromisZone(args, "spilt/promis_zone_def2/train.csv", 'train', train_transforms)
    val_set = PromisZone(args, "spilt/promis_zone_def2/val.csv", 'val', val_transforms)
    test_set = PromisZone(args, "spilt/promis_zone_def2/test.csv", 'test', test_transforms)
    train_loader = DataLoader(
        train_set,
        batch_size=args.batch_size,
        shuffle=True,
